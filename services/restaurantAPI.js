...
export const restaurantAPI = {
  ...
  searchRestaurants: async function(payload) {
    const queryParams = stringify(payload, { encodeValuesOnly: true });
    let response = await request(`/restaurants/search/?${queryParams}`);
    if (response.status.toLowerCase() === "ok") {
      return response.data;
    } else {
      return [];
  ...
  getMenus: async (restaurantId) => {
    ...
    let response = await request(`${API_URL}/restaurants/getMenus`);
    if (response.status.toLowerCase() === "ok") {
      return response.data;
    } else {
      return [];
  ...
  getCurrentRestaurant: async (payload) => {
    ...
    let response = await request(
      `${API_URL}/restaurants/get-restaurants-details`,
      {
        method: "POST",
        body: {
          res_id: payload,
