...
class Home extends Component {
    ...
    if (query) {
      ...
      restaurants = restaurants.map((rec) => {
        ...
        if (rec.restaurant_id.images.length) {
          restAvatar = `${API_IMAGE_URL}/assets/images/restaurants/${rec.restaurant_id.slug}/${rec.restaurant_id.images[0].path}`;
        }
  ...
  handleClick = (tag) => {
    ...
    if (filtered.length > 0) {
      filtered = filtered[0].dishes.map((rec) =>
        dishes.find((rec1) => rec1._id == rec._id)
      );
    }
          ...
          >
            <Grid container direction="column">
              ...
              <Grid item container direction="column">
                {dishesWithTags != null && dishesWithTags.length > 0 && (
                  <SectionHeaders
                ...
                {dishesWithTags != null && dishesWithTags.length > 0 && (
                  <Grid container direction="row">
                    <div
                      ...
                      `}
                    >
                      {dishesWithTags.map((rec) => (
                        <NewDishCard
                          classes={classes}
                          onclick={this.handleClick}
                          name={rec.tag}
                          des={rec.dishes.length + " dishes"}
                          url={`${API_IMAGE_URL}/assets/images/tags/${rec.tag
                            .replace(" ", "-")
                            .toLowerCase()}.jpg`}
                          type="dishes"
                        />
                      ...
                      {latestReviews.map((rec) => (
                        <NewDishCard
                          ...
                          url={
                            ...
                            rec.product.pimage.name +
                            "/" +
                            rec.product.pimage.path
                          }
                          classes={classes}
                        />
                      ))}
...
export default connect(
  (state) => ({
    ...
    topten:
      state.RestaurantsReducer.dishes &&
      state.RestaurantsReducer.dishes.sort((a, b) =>
        a.avgRatings < b.avgRatings ? 1 : -1
      ),
